...
file_csv = "DS2_Notes.csv"
file_bareme = "DS2_Bareme.csv"
bdd = "BDD_MP.db"
promo = 2019
num_ds = 2
...
def lire_bareme(file):
    ...
    a=fid.readline() # La premiere ligne est inutile.
    print(a)
...
def calcul_note_eleve(notes_eleve,bareme,bareme_comp):
    ...
    for i in range(len(bareme)):
        comp = bareme[i][3:]
        #print(id_eleve,i,notes,len(notes))
        #print(notes[38])
        n = notes[i]
        poids = bareme[i][1]
...
def ecriture_notes_tex(bilan_eleve,moyenne_classe,bareme,quest_comp,promo,num_ds,file_bdd):
    ...
    for clef in dico :
        ...
        req = 'INSERT INTO eleve_competences (id_eleve,promo,id_competence,reussite,ds) VALUES ('+str(id_el)+',' +str(promo)+',"' +id_comp+'",' +str(round(taux,2))+','+str(num_ds)+ ' )'
        c.execute(req)
        conn.commit()
        conn.close()
            
            
        ligne = id_comp + " -- " + nom_court + "&" + str(round(taux,2)) +" \\%" +"\\\ \\hline \n"
        fid.write(ligne)    
...
def generation_bilan(bilan_ds,moyenne_classe,bareme,quest_comp,promo,num_ds,bdd):
    for i in range(0,len(bilan_ds)):
        bilan_eleve=bilan_ds[i]
        print("eleve ",i+1," ", bilan_eleve[1],"\n")
...
def go():
    ...
    req = "DELETE FROM ds WHERE promo='"+str(promo)+"' and ds='"+str(num_ds)+"'"
    c.execute(req)
    ...
    moyenne_classe,notes_eleves = stat_ds(bilan_ds,1.1,0)
